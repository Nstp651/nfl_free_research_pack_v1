...
Call `computeNflReceptionsFreezeV5`. Require actual `complete_model_integrity_confirmed:true` and `p_model_status:FROZEN`, valid distributions and monotonic ladders before declaring freeze. Preserve receipts and any executed pre-market threshold sensitivity. No retrospective reliability relabeling.

## LAYER 3
Only after freeze call `getNflReceptionsBoardV5(run_id)`. Verify exact run/receipt/fixture/player/team/full-game Over mapping. k+ = Over k-0.5 only; no interpolation, whole-number push substitution, promos or SGM-only lines.
Retrieve the complete mapped board using documented pagination/frozen endpoints. Positive-only or missing coverage => MARKET COVERAGE INCOMPLETE and block recommendation.
CURRENT quote age=0–30m. Future/missing/noncurrent timestamps cannot create a new recommendation. Deduplicate CURRENT player+k by highest odds, newest quote, standard before alternate, book key.
Use p=P_model and o=decimal odds: implied=1/o; fair=1/p; Edge=p-1/o; ROI=p*o-1. Raw `positive_edge_ranked` is audit-only and never directly determines BEST SINGLE.
...
Complete valid inputs + no CORE => `NO BET — no qualifying actionable core reception bet.` Missing execution/evidence/coverage => exact INCOMPLETE status, not NO BET.

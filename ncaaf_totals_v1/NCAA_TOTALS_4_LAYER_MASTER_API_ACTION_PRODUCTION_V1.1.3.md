# NCAA FOOTBALL TOTALS — 4-LAYER MASTER API/ACTION PRODUCTION V1.1.3
...
V1.1.3 retains integer/half-point push-aware pricing and adds a mandatory identity-binding contract: research, QBASE and frozen P_model records are reconciled by exact `game_id` and per-game anchor hash, never by array position.
...
# V1.1.3 RUNTIME / CHECKPOINT EXECUTION CONTRACT

The existing research Worker now owns deterministic bulk execution. All earlier identity, cutoff, probability, research and market-blindness gates remain mandatory. The execution location changes; no game or gate is sampled or skipped.

1. After safe market health and full fixture/window reconciliation, call `startNcaafTotalsFreezeRun`. It resolves one immutable Git commit, reads COMPLETE research and QBASE source artifacts, verifies the entire keyed universe and every published anchor, enforces cutoff, and derives the eligible set at its fixed server timestamp. Preserve `run_id`, source commit, revisions and eligible IDs. This replaces conversational retrieval/hashing of every QBASE grid.
2. Page `getNcaafTotalsFreezeResearch` through every eligible game. It returns full structured game research plus compact QBASE anchors. Reconcile the returned IDs with the authoritative eligible set. No array-position joins.
3. Complete the mandatory current scan for BOTH teams in every game. Batch official schedule/status lookups and reuse relevant sources without duplicating searches. Retain concise evidence URLs, checked timestamps, findings and unresolved flags for EVERY required topic. Deep research remains mandatory for triggered games. A checkpoint is storage, never research-completion evidence by itself.
4. Submit complete per-game research/context records in batches of at most five via `checkpointNcaafTotalsResearch`. Save the returned run ID in conversation. If interrupted, read status and resume pending games; refresh stale or materially changed evidence. A missing topic or unknown outcome cannot be silently called verified. No invented evidence, neutral default assumptions or synthetic test receipts in real runs.
5. Once ALL IDs have completed research, call `computeNcaafTotalsFreeze`. The Worker rechecks every anchor, exact teams, revisions, full grid, context ledger, scenario weights, all probability audits and zero-shift invariants. Any started eligible game or expired evidence invalidates completion; start a new clean run rather than quietly dropping games.
6. The Worker persists all outputs atomically and returns a compact receipt with original freeze timestamp, full-slate identity/input/numerical/snapshot hashes, and per-game summaries. Declare FROZEN only on a successful response with all PASS fields and an exact eligible set. An exception, timeout, missing receipt or partial checkpoint is NOT a freeze. Retrying compute after success returns the original immutable result.
7. Only then access the AU market board. Fetch exact persisted grids using `getNcaafTotalsFrozenGame` for priced fixtures. Never reconstruct from QBASE or resubmit context after market exposure. Verify run/receipt/time on every frozen read. Apply existing exact fixture/line mapping and push-aware Layers 3/4.

Residual execution uses the unchanged QBASE artifact's calibrated empirical CDF, linearly interpolated residual quantiles and normal tails. Bias is applied once. Scenario `shift` is relative to keyed calibrated QBASE mean; `residual_scale` scales centered residual deviations (1 means unchanged). Weighted shift must equal contextual_shift within 1e-9. Multiple scenarios or any nonunit residual scale require distribution_changed=true. Every nonzero shift or changed variance needs a material ledger reference and quantified basis. A zero-shift, unchanged-distribution game copies the exact original grid, preserving its hash.

The residual bundle is generated from the existing Python artifact and pinned by its Python canonical model SHA-256. Mismatching models fail closed. Anchor/grid SHA formats remain V1.1.2-compatible. New whole-snapshot receipt hashes encode every numeric value as `f64:` plus big-endian IEEE-754 hex (zero normalized to +0), sort object keys, retain array order and hash compact UTF-8 JSON. Eligible games are sorted by exact ID before receipts; sorting never binds source records.

Infrastructure: a SQLite Durable Object attached to the existing research Worker stores each run's locked sources, research checkpoints and immutable per-game grids. The run ID is a capability; keep it private. No paid data dependency is added. Cloudflare Git integration continues to own deployment. Deploy the new binding/migration with the Worker before replacing the Custom GPT Action schema. Free-tier compatibility is a target, not a promise of unlimited capacity.

Release acceptance is separate from a numerical test: synthetic test ledgers demonstrate code behavior only. The Custom GPT is fully production-live only after a clean real research run freezes and, in the SAME conversation, real post-freeze market integration completes `FINAL_NCAAF_TOTALS_SLATE_RANKING_COMPLETE`.

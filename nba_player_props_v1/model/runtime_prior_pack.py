...
def build_runtime_prior_pack(history: pd.DataFrame, *, history_sha256: str) -> dict:
    ...
    team_keys = ["game_id_espn", "game_start_utc", "team_id_espn", "opponent_team_id_espn"]
    ...
    for player_id, rows in data.groupby("player_id_espn", sort=True):
        ...
        last = rows.iloc[-1]
        features = {
            "starter_prev": float(bool(last.starter)),
            "start_rate_l5": _mean_tail(rows, "starter", 5),
            "start_rate_l10": _mean_tail(rows, "starter", 10),
            "minutes_l3": _mean_tail(rows, "minutes", 3),
            "minutes_l5": _mean_tail(rows, "minutes", 5),
            "minutes_l10": _mean_tail(rows, "minutes", 10),
            "minutes_l20": _mean_tail(rows, "minutes", 20),
            "assists_l3": _mean_tail(rows, "assists", 3),
            "assists_l5": _mean_tail(rows, "assists", 5),
            "assists_l10": _mean_tail(rows, "assists", 10),
            "assists_l20": _mean_tail(rows, "assists", 20),
            "assists_per_min_l5": _mean_tail(rows, "assists_per_min_current", 5),
            "assists_per_min_l10": _mean_tail(rows, "assists_per_min_current", 10),
            "assist_share_l5": _mean_tail(rows, "assist_share_current", 5),
            "assist_share_l10": _mean_tail(rows, "assist_share_current", 10),
            "turnovers_l5": _mean_tail(rows, "turnovers", 5),
            "fga_l5": _mean_tail(rows, "field_goals_attempted", 5),
            "rebounds_l3": _mean_tail(rows, "rebounds", 3),
            "rebounds_l5": _mean_tail(rows, "rebounds", 5),
            "rebounds_l10": _mean_tail(rows, "rebounds", 10),
            "rebounds_l20": _mean_tail(rows, "rebounds", 20),
            "offensive_rebounds_l5": _mean_tail(rows, "offensive_rebounds", 5),
            "defensive_rebounds_l5": _mean_tail(rows, "defensive_rebounds", 5),
            "rebounds_per_min_l5": _mean_tail(rows, "rebounds_per_min_current", 5),
            "rebounds_per_min_l10": _mean_tail(rows, "rebounds_per_min_current", 10),
            "rebound_share_l5": _mean_tail(rows, "rebound_share_current", 5),
            "rebound_share_l10": _mean_tail(rows, "rebound_share_current", 10),
        }
        players[str(player_id)] = {
            "player_id": str(player_id),
            "last_team_id": str(last.team_id_espn),
            "last_opponent_team_id": str(last.opponent_team_id_espn),
            "last_game_start_utc": last.game_start_utc.isoformat(),
    ...
    for team_id, rows in team.groupby("team_id_espn", sort=True):
        ...
        teams[str(team_id)] = {
            ...
            "last_game_start_utc": last.game_start_utc.isoformat(),
            "features": {
                "team_possessions_l5": _mean_tail(rows, "game_possessions_est", 5),
                "team_possessions_l10": _mean_tail(rows, "game_possessions_est", 10),
                "team_assists_l5": _mean_tail(rows, "team_assists", 5),
                "team_assists_l10": _mean_tail(rows, "team_assists", 10),
                "team_rebounds_l5": _mean_tail(rows, "team_rebounds", 5),
                "team_rebounds_l10": _mean_tail(rows, "team_rebounds", 10),
                "assists_allowed_l5": _mean_tail(rows, "opponent_assists_current", 5),
                "assists_allowed_l10": _mean_tail(rows, "opponent_assists_current", 10),
                "rebounds_allowed_l5": _mean_tail(rows, "opponent_rebounds_current", 5),
                "rebounds_allowed_l10": _mean_tail(rows, "opponent_rebounds_current", 10),
            },
        }
    ...
    pack = {
        ...
        "max_history_season": int(data.season.max()),
...
def main() -> None:
    ...
    pack = build_runtime_prior_pack(history, history_sha256=sha256_file(args.history))
    Path(args.output).parent.mkdir(parents=True, exist_ok=True)
    Path(args.output).write_bytes(canonical_json(pack) + b"\n")

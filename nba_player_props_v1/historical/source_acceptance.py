...
# Fields available in the normalized model-training table and independent team box.
RECON_FIELDS = (
    "assists",
    "field_goals_made",
...
# Raw player reconciliation also checks the two target stats. SportsDataverse team
# boxes expose ORB + DRB rather than a direct rebounds column, so total rebounds is
# derived from those independent components before reconciliation.
RAW_RECON_FIELDS = (
    "assists",
    "rebounds",
    "offensive_rebounds",
...
TEAM_BOX_SOURCE_FIELDS = tuple(field for field in RAW_RECON_FIELDS if field != "rebounds")
...
def _download(url: str, cache: Path) -> tuple[Path, str, int]:
    ...
    if not path.exists():
        req = Request(
            url,
            headers={
                "User-Agent": "NBA-V1-source-acceptance/1.0",
...
def _asset_for(season: int, dataset: str):
    matches = [asset for asset in core_assets(season) if asset.dataset == dataset]
    if len(matches) != 1:
...
def _first_column(frame: pd.DataFrame, candidates: tuple[str, ...], label: str) -> str:
    ...
    if not hits:
        raise ValueError(
            f"{label} schema missing one of {list(candidates)}; "
...
def _normalize_team_box(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in ("game_id", "team_id", "opponent_team_id"):
        out[column] = out[column].astype("string")
    out = out[
        out.season_type.isin([2, 3])
        & out.team_id.isin(NBA_ESPN_TEAMS)
        & out.opponent_team_id.isin(NBA_ESPN_TEAMS)
    ].copy()
    for column in ("team_score", "opponent_team_score", *TEAM_BOX_SOURCE_FIELDS):
        out[column] = pd.to_numeric(out[column], errors="raise")
        if (out[column] < 0).any():
            raise ValueError(f"negative team-box {column}")

    out["rebounds"] = out["offensive_rebounds"] + out["defensive_rebounds"]
...
def _normalize_raw_player_box(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    out = frame.loc[:, sorted(required)].copy()
    for column in ("game_id", "athlete_id", "team_id", "opponent_team_id"):
        out[column] = out[column].astype("string")
    out = out[
        out.season_type.isin([2, 3])
        & out.team_id.isin(NBA_ESPN_TEAMS)
    ...
    for column in RAW_RECON_FIELDS:
        out[column] = pd.to_numeric(out[column], errors="coerce").fillna(0)
        if (out[column] < 0).any():
            raise ValueError(f"negative raw player-box {column}")
    if not np.isclose(
        out["rebounds"].astype(float),
        (out["offensive_rebounds"] + out["defensive_rebounds"]).astype(float),
        atol=0,
        rtol=0,
    ).all():
        raise ValueError("raw player-box ORB + DRB != REB")
...
def _normalize_schedule(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    time_col = _first_column(
        frame, ("game_date_time", "date", "game_date"), "schedule start time"
    )
    out = frame.loc[
        :, ["game_id", "season", "season_type", home_col, away_col, time_col]
    ].copy().rename(
        columns={
            home_col: "home_team_id",
            away_col: "away_team_id",
            time_col: "game_date_time",
        }
    )
    for column in ("game_id", "home_team_id", "away_team_id"):
        out[column] = out[column].astype("string")
    out = out[
        out.season_type.isin([2, 3])
        & out.home_team_id.isin(NBA_ESPN_TEAMS)
        & out.away_team_id.isin(NBA_ESPN_TEAMS)
    ].copy()
...
def _field_reconciliation(
    merged: pd.DataFrame, field: str, player_prefix: str = "player_"
) -> dict:
    ...
    delta = player - team
    mismatch = merged.loc[
        ~ok, ["game_id_espn", "team_id_espn", "season", player_col, field]
    ].copy()
    ...
    return {
        ...
        "mean_abs_delta_mismatches": (
            float(delta.loc[~ok].abs().mean()) if (~ok).any() else 0.0
        ),
...
def reconcile(history: pd.DataFrame, team_box: pd.DataFrame, schedule: pd.DataFrame) -> dict:
    """Strict diagnostic reconciliation of the model-training table."""
    history = history.copy()
    for column in ("game_id_espn", "team_id_espn", "opponent_team_id_espn"):
        history[column] = history[column].astype("string")
    history["game_start_utc"] = pd.to_datetime(
        history.game_start_utc, utc=True, errors="raise"
    )

    grouped = history.groupby(
        ["season", "game_id_espn", "team_id_espn"], as_index=False
    ).agg(
    ...
    team = team_box.rename(
        columns={
            "game_id": "game_id_espn",
            "team_id": "team_id_espn",
            "opponent_team_id": "team_box_opponent_team_id",
        }
    )
    merged = grouped.merge(
        team,
        on=["season", "game_id_espn", "team_id_espn"],
        how="outer",
        indicator=True,
        validate="one_to_one",
    )
    if not merged._merge.eq("both").all():
        sample = merged.loc[
            merged._merge != "both",
            ["season", "game_id_espn", "team_id_espn", "_merge"],
        ].head(20).to_dict("records")
        raise ValueError(f"team-box coverage mismatch: {sample}")

    exact = {field: _field_reconciliation(merged, field) for field in RECON_FIELDS}
    score_ok = np.isclose(
        merged.player_team_score.astype(float), merged.team_score.astype(float), atol=0, rtol=0
    )
    opponent_score_ok = np.isclose(
        merged.player_opponent_team_score.astype(float),
        merged.opponent_team_score.astype(float),
        atol=0,
        rtol=0,
    )
    opponent_id_ok = merged.opponent_team_id_espn.astype(str).eq(
        merged.team_box_opponent_team_id.astype(str)
    )

    sched = schedule.rename(columns={"game_id": "game_id_espn"}).copy()
    hist_games = history.groupby(["season", "game_id_espn"], as_index=False).agg(
        game_start_utc=("game_start_utc", "first"),
        teams=("team_id_espn", lambda series: tuple(sorted(set(map(str, series))))),
    )
    sched["teams"] = sched.apply(
        lambda row: tuple(sorted((str(row.home_team_id), str(row.away_team_id)))), axis=1
    )
    schedule_ids = set(zip(sched.season.astype(int), sched.game_id_espn.astype(str)))
    history_ids = set(
        zip(hist_games.season.astype(int), hist_games.game_id_espn.astype(str))
    )
    extra_schedule_ids = sorted(schedule_ids - history_ids)
    schedule_merge = hist_games.merge(
        sched[["season", "game_id_espn", "game_date_time", "teams"]],
        on=["season", "game_id_espn"],
        how="left",
        suffixes=("_history", "_schedule"),
        indicator=True,
        validate="one_to_one",
    )
    if not schedule_merge._merge.eq("both").all():
        sample = schedule_merge.loc[
            schedule_merge._merge != "both", ["season", "game_id_espn", "_merge"]
        ].head(20).to_dict("records")
        raise ValueError(f"played-game schedule coverage mismatch: {sample}")

    team_identity_ok = schedule_merge.teams_history.eq(schedule_merge.teams_schedule)
    seconds = (
        pd.to_datetime(schedule_merge.game_start_utc, utc=True)
        - pd.to_datetime(schedule_merge.game_date_time, utc=True)
    ).dt.total_seconds().abs()
    time_ok = seconds <= 60
    hard_rates = [value["rate"] for value in exact.values()] + [
        float(score_ok.mean()),
        float(opponent_score_ok.mean()),
        float(opponent_id_ok.mean()),
        float(team_identity_ok.mean()),
        float(time_ok.mean()),
    ]
    ...
    return {
        ...
        "played_schedule_games": int(len(schedule_merge)),
        "schedule_rows_total": int(len(sched)),
        "schedule_rows_without_played_box": int(len(extra_schedule_ids)),
        "schedule_rows_without_played_box_sample": [
            {"season": int(season), "game_id_espn": game_id}
            for season, game_id in extra_schedule_ids[:20]
        ],
        ...
        "played_schedule_coverage_rate": float(schedule_merge._merge.eq("both").mean()),
        ...
        "threshold": 0.999,
        "directionality_rule": (
            "ALL_PLAYED_GAMES_MUST_RECONCILE; UNPLAYED_SCHEDULE_ROWS_ARE_REPORTED_"
            "NOT_TREATED_AS_BOX_FAILURES"
        ),
    }


def reconcile_raw_player_to_team(raw_player: pd.DataFrame, team_box: pd.DataFrame) -> dict:
    grouped = raw_player.groupby(
        ["season", "game_id", "team_id"], as_index=False
    ).agg(
    ...
    if not coverage_ok.all():
        sample = merged.loc[
            ~coverage_ok, ["season", "game_id_espn", "team_id_espn", "_merge"]
        ].head(50).to_dict("records")
        return {
            "status": "FAIL",
            "coverage_rate": float(coverage_ok.mean()),
            "coverage_mismatch_sample": sample,
        }

    exact = {
        field: _field_reconciliation(merged, field, player_prefix="raw_")
        for field in RAW_RECON_FIELDS
    }
    opponent_ok = merged.raw_opponent_team_id.astype(str).eq(
        merged.team_box_opponent_team_id.astype(str)
    )
    passed = all(value["rate"] == 1.0 for value in exact.values()) and bool(
        opponent_ok.all()
    )
    return {
        "status": "PASS" if passed else "FAIL",
...
def quarantine_transform_audit(history: pd.DataFrame, raw_player: pd.DataFrame) -> dict:
    history = history.copy()
    for column in ("game_id_espn", "team_id_espn"):
        history[column] = history[column].astype("string")
    quarantine = raw_player[
        (raw_player.positive_stat_sum > 0)
        & (raw_player.dnp_parsed | (raw_player.minutes_parsed <= 0))
    ].copy()

    raw_grouped = raw_player.groupby(
        ["season", "game_id", "team_id"], as_index=False
    ).agg(
        **{f"raw_{field}": (field, "sum") for field in RAW_RECON_FIELDS}
    ).rename(columns={"game_id": "game_id_espn", "team_id": "team_id_espn"})
    ...
    if quarantine.empty:
        quarantine_grouped = raw_grouped[
            ["season", "game_id_espn", "team_id_espn"]
        ].copy()
        for field in RAW_RECON_FIELDS:
            quarantine_grouped[f"quarantine_{field}"] = 0.0
    else:
        quarantine_grouped = quarantine.groupby(
            ["season", "game_id", "team_id"], as_index=False
        ).agg(
            **{f"quarantine_{field}": (field, "sum") for field in RAW_RECON_FIELDS}
        ).rename(columns={"game_id": "game_id_espn", "team_id": "team_id_espn"})

    merged = raw_grouped.merge(
        history_grouped,
        on=["season", "game_id_espn", "team_id_espn"],
        how="left",
        validate="one_to_one",
    ...
    target_units = {
        field: float(quarantine[field].sum()) for field in ("assists", "rebounds")
    }
    passed = (
        all(value["rate"] == 1.0 for value in exact.values())
        and fraction <= MAX_QUARANTINED_ROW_FRACTION
    )
    return {
        "status": "PASS" if passed else "FAIL",
        ...
        "quarantine_rule": (
            "POSITIVE BOX STATS WITH DNP=true OR minutes<=0 ARE SOURCE-DEFECT ROWS; "
            "NEVER IMPUTE MINUTES INTO MODEL TRAINING"
        ),
    }
...
def runtime_espn_schema_probe(as_of: str) -> dict:
    ...
    try:
        ...
        for event in events:
            ...
            if len(competitions) != 1 or len(competitions[0].get("competitors") or []) != 2:
...
def _pinned_player_assets(source_pins_path: str | None) -> dict[int, dict]:
    ...
    for item in body.get("assets", []):
        if item.get("dataset") == "player_box":
            result[int(item["season_end_year"])] = item
    return result
...
def run(
    ...
    output: str,
    as_of: str,
    source_pins_path: str | None = None,
) -> dict:
    history = pd.read_csv(
        history_path,
        dtype={
    ...
    identity_pass = all(
        training_diagnostic[key] == 1.0
        for key in (
            "team_score_rate",
            "opponent_team_score_rate",
            "opponent_identity_rate",
            "played_schedule_coverage_rate",
            "schedule_team_identity_rate",
            "schedule_start_time_within_60s_rate",
        )
    )
    runtime_probe = runtime_espn_schema_probe(as_of)
    status = (
        "PASS"
        if raw_reconciliation["status"] == "PASS"
        and quarantine["status"] == "PASS"
        and identity_pass
        and runtime_probe["status"] == "PASS"
        else "FAIL"
    )

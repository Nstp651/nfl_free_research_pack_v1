...
function validateSnapshotFreeze(state,snapshot){need(snapshot&&snapshot.freeze_receipt_sha256===state.freeze_identity.freeze_receipt_sha256,'market snapshot freeze receipt mismatch');need(Number.isFinite(Date.parse(String(snapshot.captured_at||''))),'market snapshot captured_at invalid');}
...
export function rankCurrentMarket(state,freeze){
  ...
  for(const quote of best){const row=index.get(`${quote.game_id}|${quote.player_id}|${quote.stat_type}`);if(!row){issues.push({type:'NO_FROZEN_HEAD',quote});continue;}try{const e=evaluateExactMarket(quote,row.head.probability_grid);evaluated.push({...e,team:row.player.team,confidence:row.player.confidence,fragility:row.player.fragility,model_mean:row.head.final_mean,freeze_receipt_sha256:freeze.freeze_receipt_sha256});}catch(error){issues.push({type:'EXACT_THRESHOLD_UNAVAILABLE',game_id:quote.game_id,player_id:quote.player_id,stat_type:quote.stat_type,threshold:quote.threshold,bookmaker:quote.bookmaker,error:error.message});}}

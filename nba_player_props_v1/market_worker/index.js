...
import { createMarketRun, reconcileMarketGrant, applyApiSnapshot, applyManualSnapshot, rankCurrentMarket, marketStateSummary } from './market_run_core.js';
  ...
  async fetch(request){
    try{
      ...
      if(request.method==='POST'&&path==='/refresh'){
        const b=await body(request);exactKeys(b,['regions'],'market refresh');const regions=String(b.regions||this.env.ODDS_API_REGIONS||'us');const snapshot=await fetchOddsApiSlate({apiKey:this.env.ODDS_API_KEY,freeze:context.freeze,grant:context.grant,regions,capturedAt:new Date().toISOString()});state=await applyApiSnapshot(state,snapshot);await this.storage.put('state',state);const rankings=rankCurrentMarket(state,context.freeze);return response({run_id:runId,layer:'3-4',freeze_receipt_sha256:context.freeze.freeze_receipt_sha256,refresh:{captured_at:snapshot.captured_at,quote_count:snapshot.quotes.length,issue_count:snapshot.issues.length,event_resolution:snapshot.event_resolution,quota:snapshot.quota},rankings});
      }
      if(request.method==='POST'&&path==='/manual-quotes'){
        const b=await body(request);exactKeys(b,['source_type','captured_at','evidence_id','freeze_receipt_sha256','quotes'],'manual market refresh');state=await applyManualSnapshot(state,context.freeze,b);await this.storage.put('state',state);return response({run_id:runId,layer:'3-4',freeze_receipt_sha256:context.freeze.freeze_receipt_sha256,manual_snapshot:{captured_at:state.current_manual_snapshot.captured_at,snapshot_sha256:state.current_manual_snapshot.snapshot_sha256,quote_count:state.current_manual_snapshot.quotes.length,evidence_id:state.current_manual_snapshot.evidence_id},rankings:rankCurrentMarket(state,context.freeze)});
      }
...
export default {
  async fetch(request,env){
    try{
      if(request.method==='OPTIONS')return new Response(null,{status:204,headers:cors});authorize(request,env);const url=new URL(request.url);if(request.method==='GET'&&url.pathname==='/health')return response({ok:true,service:'nba-player-props-market',version:'1.0.0',post_freeze_only:true,markets:['player_assists','player_assists_alternate','player_rebounds','player_rebounds_alternate'],research_service_binding:Boolean(env.RESEARCH_SERVICE),durable_object_binding:Boolean(env.MARKET_RUNS),odds_api_key_configured:Boolean(env.ODDS_API_KEY)});const routed=await route(request,env,url);if(routed)return routed;return response({error:'Not found'},404);
    }catch(e){return response({error:e.message},e.message==='Unauthorized'?401:422);}
  }
};

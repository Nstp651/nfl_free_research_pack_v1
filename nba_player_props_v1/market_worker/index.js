...
function apiRefreshSummary(snapshot,replayed){return {refresh_request_id:snapshot.refresh_request_id,request_payload_sha256:snapshot.request_payload_sha256,replayed,captured_at:snapshot.captured_at,quote_count:snapshot.quotes.length,issue_count:(snapshot.issues||[]).length,event_resolution:snapshot.event_resolution||{},quota:snapshot.quota||[]};}
function manualRefreshSummary(snapshot,replayed){return {refresh_request_id:snapshot.refresh_request_id,request_payload_sha256:snapshot.request_payload_sha256,replayed,captured_at:snapshot.captured_at,snapshot_sha256:snapshot.snapshot_sha256,quote_count:snapshot.quotes.length,evidence_id:snapshot.evidence_id};}
function manualRequestPayload(body){const out=structuredClone(body);delete out.refresh_request_id;return out;}
  ...
  async fetch(request){
    try{
      ...
      if(request.method==='POST'&&path==='/refresh'){
        const b=await body(request);exactKeys(b,['regions','refresh_request_id'],'market refresh');const regions=String(b.regions||this.env.ODDS_API_REGIONS||'au'),requestPayload={regions};const replay=await marketRefreshReplay(state,'api',b.refresh_request_id,requestPayload);if(replay){await this.storage.put('state',state);return response({run_id:runId,layer:'3-4',freeze_receipt_sha256:context.freeze.freeze_receipt_sha256,refresh:apiRefreshSummary(replay.snapshot,true),rankings:rankCurrentMarket(state,context.freeze)});}
        const snapshot=await fetchOddsApiSlate({apiKey:this.env.ODDS_API_KEY,freeze:context.freeze,grant:context.grant,regions,capturedAt:new Date().toISOString()});state=await applyApiSnapshot(state,snapshot,{refreshRequestId:b.refresh_request_id,requestPayload});await this.storage.put('state',state);const rankings=rankCurrentMarket(state,context.freeze);return response({run_id:runId,layer:'3-4',freeze_receipt_sha256:context.freeze.freeze_receipt_sha256,refresh:apiRefreshSummary(state.current_api_snapshot,false),rankings});
      }
      if(request.method==='POST'&&path==='/manual-quotes'){
        const b=await body(request);exactKeys(b,['refresh_request_id','source_type','captured_at','evidence_id','freeze_receipt_sha256','quotes'],'manual market refresh');const replay=await marketRefreshReplay(state,'manual',b.refresh_request_id,manualRequestPayload(b));if(replay){await this.storage.put('state',state);return response({run_id:runId,layer:'3-4',freeze_receipt_sha256:context.freeze.freeze_receipt_sha256,manual_snapshot:manualRefreshSummary(replay.snapshot,true),rankings:rankCurrentMarket(state,context.freeze)});}

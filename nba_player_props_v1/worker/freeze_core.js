...
export async function computeSlateFreeze({slateDateEt,runMode,eligibleGameIds,researchByGame,qbaseArtifacts,priorsByGame,frozenAt}){
  ...
  const core={schema_version:'nba_slate_freeze_v1',market_data:false,status:'FROZEN',slate_date_et:slateDateEt,run_mode:runMode,eligible_game_ids:ids,games,qbase_heads:qbaseHeads,frozen_at:frozenAt||new Date().toISOString()};core.freeze_receipt_sha256=await sha256Json(core);return core;
}

...
def _validate_scenarios(head: dict[str, Any], known_sources: set[str], label: str) -> tuple[list[dict[str, Any]], float]:
    ...
    for idx, row in enumerate(scenarios):
        ...
        seen_ids.add(sid)
        try:
            weight = float(row["weight"]); mean = float(row["mean"])
        except (KeyError, TypeError, ValueError) as exc:
        ...
        if receipt and not re.fullmatch(r"[0-9a-f]{64}", receipt):
            raise ValueError(f"{label}.scenarios[{idx}] quant_input_receipt_sha256 invalid")
        out.append({
            "id": sid,
            "weight": weight,
            "mean": mean,
            ...
            "quant_input_receipt_sha256": receipt or None,
        })
        total_weight += weight
...
def build_frozen_matchup(
    ...
    *,
    ...
    for pidx, projection in enumerate(player_projections):
        ...
        if set(heads) - set(supplied_heads):
            raise ValueError(f"modeled player {key} missing requested heads {sorted(set(heads)-set(supplied_heads))}")
        # In single-stat mode we reject accidental second-head execution so freeze
        # semantics stay explicit and reproducible.
        ...
        for stat in heads:
            ...
            contract = qbase[stat]
            grid = probability_grid(final_mean, contract["dispersion_alpha"], contract["max_count"])
            frozen_heads[stat] = {
                "qbase_anchor": contract,
                "qbase_mean": qbase_mean,
                "scenario_ledger": scenarios,
                "final_mean": final_mean,
                "dispersion_alpha": contract["dispersion_alpha"],

...
async function sha256WireJson(x){const bytes=new TextEncoder().encode(JSON.stringify(x));const hash=await crypto.subtle.digest('SHA-256',bytes);return [...new Uint8Array(hash)].map(v=>v.toString(16).padStart(2,'0')).join('');}
...
async function harness(){
  ...
  const qA={model_name:'assists q',model_version:'0.1.0',feature_schema:'nbl_player_pregame_v1',stat_type:'assists',market_data:false,selected_model:selected(Math.log(2)),walk_forward:{nb2_alpha_oos:0.2},probability_contract:{max_count:20}};
  const qR={model_name:'rebounds q',model_version:'0.1.0',feature_schema:'nbl_player_pregame_v1',stat_type:'rebounds',market_data:false,selected_model:selected(Math.log(4)),walk_forward:{nb2_alpha_oos:0.25},probability_contract:{max_count:30}};
...
test('persistent run initializes, publishes server QBASE, checkpoints, freezes and preserves receipt',async()=>{
  const h=await harness();try{
    ...
    res=await req(run,'POST','/compute',{projections:[]});d=await res.json();assert.equal(d.frozen_at,stamp);assert.equal(d.freeze_receipt_sha256,receipt);assert.equal(d.players[0].player_model_sha256,playerHash);
    res=await req(run,'GET','/players/id%3Ap1');d=await res.json();assert.equal(d.player.player_name,'Test Guard');assert.equal(d.freeze_receipt_sha256,receipt);assert.equal(d.player_model_sha256,playerHash);assert.equal(await sha256Json(d.player),playerHash);assert.ok(d.player.heads.assists.probability_grid.half_point_grid.length>0);
  }finally{h.restore();}
});

test('new-to-NBL heads get deterministic conservative server dispersion when client cannot estimate it',async()=>{
  const h=await harness();try{const run=new NblMatchRun(new MemoryState(),{});await req(run,'POST','',{fixture_id:'fixture-1',season_start:2026,run_mode:'BOTH'});await req(run,'POST','/research',importResearch(h.manifest.asset_revision));let res=await req(run,'POST','/compute',{projections:importProjections()});assert.equal(res.status,200);res=await req(run,'GET','/players/id%3Ap2');assert.equal(res.status,200);const d=await res.json(),p=d.player;assert.equal(p.player_name,'Away Guard');for(const stat of ['assists','rebounds']){assert.equal(p.heads[stat].server_quantitative_attestation.source,'PRIOR_COMP_TRANSLATION');assert.equal(p.heads[stat].dispersion_source,'MAX_QBASE_PRIOR_COMP');assert.equal(p.heads[stat].confidence,'C');assert.equal(p.heads[stat].fragility,'HIGH');assert.ok(p.heads[stat].dispersion_alpha>=(stat==='assists'?0.35:0.30));}}
  finally{h.restore();}
});

test('server rejects a client attempt to alter returning-player QBASE mean',async()=>{
  const h=await harness();try{const run=new NblMatchRun(new MemoryState(),{});await req(run,'POST','',{fixture_id:'fixture-1',season_start:2026,run_mode:'BOTH'});await req(run,'POST','/research',research(h.manifest.asset_revision));const bad=projections();bad[0].heads.rebounds.qbase_mean=99;const res=await req(run,'POST','/compute',{projections:bad});assert.equal(res.status,422);assert.match((await res.json()).error,/client qbase_mean does not match server QBASE/);}finally{h.restore();}
});

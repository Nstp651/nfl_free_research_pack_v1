...
async function pinnedPrior(meta){
...
async function baselineHeads(storage,meta,prior,playerName,team){
  const out={};
  for(const stat of headsFor(meta.lock.run_mode)){
    const qbase=await storage.get('q:'+stat);requireThat(qbase,`Pinned ${stat} QBASE missing`);
    try{
      const b=await returningPlayerBaseline(qbase,prior,{playerName,team,fixture:meta.lock.fixture,targetSeasonStart:meta.lock.season_start});
      out[stat]={status:'SERVER_QBASE_RUNTIME_SCORE',mean:b.mean,quant_input_receipt_sha256:b.quant_input_receipt_sha256,missing_features:b.missing_features};
    }catch(e){
      if(e?.code==='PRIOR_COMP_TRANSLATION_REQUIRED')out[stat]={status:'PRIOR_COMP_TRANSLATION_REQUIRED'};else throw e;
    }
  }
  return out;
}
...
async function entryResearchSeed(meta,storage){
  const {prior}=await pinnedPrior(meta),f=meta.lock.fixture,season=meta.lock.season_start;
  const [home,away]=await Promise.all([safeRoster(f.home_team.id,season),safeRoster(f.away_team.id,season)]);
  const decorate=async(rows,team)=>Promise.all(rows.map(async p=>{const priorNbl=compactPrior(prior,p.display_name);return {...p,team,prior_nbl:priorNbl,nbl_history_status:priorNbl?'NBL_HISTORY_AVAILABLE':'PRIOR_COMP_TRANSLATION_REQUIRED',qbase_baseline:await baselineHeads(storage,meta,prior,p.display_name,team)};}));
...
async function serverizeHead(qbase,prior,meta,researchPlayer,head,stat){
  requireThat(head&&typeof head==='object',`${researchPlayer.player_name}.${stat} head required`);
  let baseline=null;
  try{
    baseline=await returningPlayerBaseline(qbase,prior,{playerName:researchPlayer.player_name,team:researchPlayer.team,fixture:meta.lock.fixture,targetSeasonStart:meta.lock.season_start});
  }catch(e){if(e?.code!=='PRIOR_COMP_TRANSLATION_REQUIRED')throw e;}
  requireThat(Array.isArray(head.scenarios)&&head.scenarios.length>0,`${researchPlayer.player_name}.${stat} scenarios required`);
  const scenarios=[];
  if(baseline){
    ...
    for(const original of head.scenarios){
      ...
      if(method==='QBASE_RUNTIME_SCORE'){
        if(s.mean!==undefined&&s.mean!==null)requireThat(finite(s.mean)&&close(s.mean,baseline.mean),`${researchPlayer.player_name}.${stat}.${s.id} runtime mean mismatch`);
        s.mean=baseline.mean;s.quant_input_receipt_sha256=baseline.quant_input_receipt_sha256;
      }else if(method==='QBASE_MINUTES_RECOMPUTE'){
        requireThat(finite(s.projected_minutes),`${researchPlayer.player_name}.${stat}.${s.id} projected_minutes required for server recompute`);
        const moved=await projectedMinutesScore(qbase,baseline.resolved_features,Number(s.projected_minutes),{starterProbability:s.starter_probability??null});
        if(s.mean!==undefined&&s.mean!==null)requireThat(finite(s.mean)&&close(s.mean,moved.mean),`${researchPlayer.player_name}.${stat}.${s.id} minutes mean mismatch`);
        s.mean=moved.mean;s.quant_input_receipt_sha256=moved.quant_input_receipt_sha256;
      }else{
        requireThat(['EMPIRICAL_ROLE_SPLIT','PRIOR_COMP_TRANSLATION'].includes(method),`${researchPlayer.player_name}.${stat}.${s.id} unsupported scenario method`);
        requireThat(finite(s.mean),`${researchPlayer.player_name}.${stat}.${s.id} external scenario mean required`);
        requireThat(/^[0-9a-f]{64}$/.test(String(s.quant_input_receipt_sha256||'')),`${researchPlayer.player_name}.${stat}.${s.id} external quant receipt required`);
      }
      scenarios.push(s);
    }
    return {...head,qbase_mean:baseline.mean,server_qbase_source:'SERVER_QBASE_RUNTIME_SCORE',server_qbase_receipt_sha256:baseline.quant_input_receipt_sha256,scenarios};
  }
  for(const original of head.scenarios){
    const s={...original},method=String(s.method||'').toUpperCase();requireThat(method==='PRIOR_COMP_TRANSLATION',`${researchPlayer.player_name}.${stat} has no NBL prior; all scenarios must use PRIOR_COMP_TRANSLATION`);requireThat(finite(s.mean),`${researchPlayer.player_name}.${stat}.${s.id} translated mean required`);requireThat(/^[0-9a-f]{64}$/.test(String(s.quant_input_receipt_sha256||'')),`${researchPlayer.player_name}.${stat}.${s.id} translation quant receipt required`);scenarios.push(s);
  }
  const weight=scenarios.reduce((a,s)=>a+Number(s.weight||0),0);requireThat(Math.abs(weight-1)<=1e-9,`${researchPlayer.player_name}.${stat} translation weights must sum to 1`);const translated=scenarios.reduce((a,s)=>a+Number(s.weight)*Number(s.mean),0);
  requireThat(head.dispersion_override&&finite(head.dispersion_override.alpha),`${researchPlayer.player_name}.${stat} prior-comp translation requires explicit dispersion override`);
  return {...head,qbase_mean:translated,server_qbase_source:'PRIOR_COMP_TRANSLATION',server_qbase_receipt_sha256:null,scenarios};
}

async function serverizeProjections(meta,storage,research,projections){
  requireThat(Array.isArray(projections)&&projections.length>0,'projections required');const {prior}=await pinnedPrior(meta),qbase={};for(const stat of headsFor(meta.lock.run_mode)){qbase[stat]=await storage.get('q:'+stat);requireThat(qbase[stat],`Pinned ${stat} QBASE missing`);}
  const researchById=new Map(),researchByNameTeam=new Map();for(const p of research.players){if(String(p.player_id||'').trim())researchById.set(String(p.player_id).trim(),p);researchByNameTeam.set(`${norm(p.player_name)}:${norm(p.team)}`,p);}
  const out=[];
...
/** Stable Cloudflare entry class. The base run owns durable storage and atomic freeze.
 * This wrapper reconstructs pinned research assets, publishes server-computed QBASE
 * baselines, and makes server-side quantitative recomputation authoritative before
 * delegating the final atomic freeze to the base implementation. */

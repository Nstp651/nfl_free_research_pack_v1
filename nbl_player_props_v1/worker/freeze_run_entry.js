...
async function entryResearchSeed(meta,storage){
  ...
  const decorate=async(rows,team)=>Promise.all(rows.map(async p=>{const priorNbl=compactPrior(prior,p.display_name,p.id),priority=researchPriority(priorNbl,team);return {...p,team,prior_nbl:priorNbl,...priority,nbl_history_status:priorNbl?'NBL_HISTORY_AVAILABLE':'PRIOR_COMP_TRANSLATION_REQUIRED',qbase_baseline:await baselineHeads(storage,meta,prior,p.display_name,p.id,team)};}));
  return {market_data:false,lock:meta.lock,fixture:f,home_roster:await decorate(home,f.home_team.name),away_roster:await decorate(away,f.away_team.name),snapshot_revision:prior.snapshot_revision,research_note:'Structured data is a prior only. Screen the full roster once, then deep-research DEEP-priority players plus any SCREEN/LOW player promoted by current role evidence. Current availability, projected minutes, role, lineup, imports, coaching and late news still require current pre-market research. Returning-player QBASE baseline means are computed and opening-season stabilized server-side; new imports require explicit prior-competition translation while missing dispersion overrides are supplied conservatively by the server.'};
}

async function serverPriorCompDispersion(qbase,stat,scenarios){
  const qbaseAlpha=Number(qbase?.walk_forward?.nb2_alpha_oos);requireThat(Number.isFinite(qbaseAlpha)&&qbaseAlpha>0,`${stat} QBASE alpha missing for prior-comp fallback`);const floor=stat==='assists'?0.35:0.30,alpha=Math.min(5,Math.max(qbaseAlpha,floor,qbaseAlpha*2));const receipt=await sha256Json({policy:'SERVER_PRIOR_COMP_DISPERSION_V1',stat_type:stat,qbase_model_version:qbase.model_version,qbase_alpha:qbaseAlpha,alpha,translation_receipts:scenarios.map(s=>String(s.quant_input_receipt_sha256||''))});return {method:'MAX_QBASE_PRIOR_COMP',alpha,receipt_sha256:receipt,server_policy:'SERVER_PRIOR_COMP_DISPERSION_V1'};
}
...
async function serverizeHead(qbase,prior,meta,researchPlayer,head,stat){
  ...
  if(baseline){
    const priorPlayer=priorFor(prior,researchPlayer.player_name,researchPlayer.player_id);
    ...
    for(const original of head.scenarios){
      const s={...original},method=String(s.method||'').toUpperCase();
      if(method==='QBASE_RUNTIME_SCORE'){
        if(s.mean!==undefined&&s.mean!==null)requireThat(finite(s.mean)&&close(s.mean,baseline.mean),`${researchPlayer.player_name}.${stat}.${s.id} runtime mean mismatch`);s.mean=baseline.mean;s.quant_input_receipt_sha256=baseline.quant_input_receipt_sha256;if(baseline.opening_stabilization?.applied)s.assumptions=[...(Array.isArray(s.assumptions)?s.assumptions:[]),`SERVER_QBASE_OPENING_STABILIZATION raw=${baseline.raw_mean} anchor=${baseline.opening_stabilization.anchor} stabilized=${baseline.mean}`];
      }else if(method==='QBASE_MINUTES_RECOMPUTE'){
        requireThat(finite(s.projected_minutes),`${researchPlayer.player_name}.${stat}.${s.id} projected_minutes required for server recompute`);const moved=await projectedMinutesScore(qbase,baseline.resolved_features,Number(s.projected_minutes),{starterProbability:s.starter_probability??null});if(s.mean!==undefined&&s.mean!==null)requireThat(finite(s.mean)&&close(s.mean,moved.mean),`${researchPlayer.player_name}.${stat}.${s.id} minutes mean mismatch`);s.mean=moved.mean;s.quant_input_receipt_sha256=moved.quant_input_receipt_sha256;if(moved.opening_stabilization?.applied)s.assumptions=[...(Array.isArray(s.assumptions)?s.assumptions:[]),`SERVER_QBASE_OPENING_STABILIZATION raw=${moved.raw_mean} anchor=${moved.opening_stabilization.anchor} stabilized=${moved.mean}`];
      }else{
        requireThat(['EMPIRICAL_ROLE_SPLIT','PRIOR_COMP_TRANSLATION'].includes(method),`${researchPlayer.player_name}.${stat}.${s.id} unsupported scenario method`);requireThat(finite(s.mean),`${researchPlayer.player_name}.${stat}.${s.id} external scenario mean required`);requireThat(/^[0-9a-f]{64}$/.test(String(s.quant_input_receipt_sha256||'')),`${researchPlayer.player_name}.${stat}.${s.id} external quant receipt required`);
      }
      const sanity=projectionSanity({playerName:researchPlayer.player_name,stat,method,mean:s.mean,priorPlayer,projectedMinutes:s.projected_minutes??researchPlayer.projected_minutes?.mean,seasonGamesPrior:baseline.resolved_features?.player_season_games_prior??0,confidence:head.confidence,fragility:head.fragility});
      requireThat(sanity.status!=='FAIL',sanity.error||`${researchPlayer.player_name}.${stat} projection sanity failed`);
      if(sanity.status==='PASS'){const note=`SERVER_PROJECTION_SANITY_PASS anchor=${sanity.anchor} stable_ceiling=${sanity.stable_ceiling} recent_peak=${sanity.recent_peak??'na'} rate_anchor=${sanity.rate_anchor??'na'} opening=${sanity.opening_regime}`;s.assumptions=[...(Array.isArray(s.assumptions)?s.assumptions:[]),note];}
      scenarios.push(s);
    }
    return {...head,qbase_mean:baseline.mean,server_qbase_raw_mean:baseline.raw_mean,server_qbase_opening_stabilization:baseline.opening_stabilization,server_qbase_source:'SERVER_QBASE_RUNTIME_SCORE',server_qbase_receipt_sha256:baseline.quant_input_receipt_sha256,server_player_prior_key:baseline.feature_context.player_prior_key,scenarios};
  }
  ...
  const weight=scenarios.reduce((a,s)=>a+Number(s.weight||0),0);requireThat(Math.abs(weight-1)<=1e-9,`${researchPlayer.player_name}.${stat} translation weights must sum to 1`);const translated=scenarios.reduce((a,s)=>a+Number(s.weight)*Number(s.mean),0),serverFallback=!head.dispersion_override,dispersion_override=head.dispersion_override||await serverPriorCompDispersion(qbase,stat,scenarios);return {...head,confidence:serverFallback?'C':head.confidence,fragility:serverFallback?'HIGH':head.fragility,dispersion_override,qbase_mean:translated,server_qbase_source:'PRIOR_COMP_TRANSLATION',server_qbase_receipt_sha256:null,server_player_prior_key:null,scenarios};
}

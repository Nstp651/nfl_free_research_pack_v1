...
async function entryResearchSeed(meta,storage){
  ...
  return {market_data:false,lock:meta.lock,fixture:f,home_roster:await decorate(home,f.home_team.name),away_roster:await decorate(away,f.away_team.name),snapshot_revision:prior.snapshot_revision,research_note:'Structured data is a prior only. Screen the full roster once, then deep-research DEEP-priority players plus any SCREEN/LOW player promoted by current role evidence. Current availability, projected minutes, role, lineup, imports, coaching and late news still require current pre-market research. Returning-player QBASE baseline means are computed server-side; new imports require explicit prior-competition translation.'};
}

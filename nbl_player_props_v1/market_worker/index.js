...
async function oddsFetch(url,label){const res=await fetch(url,{headers:{accept:'application/json','user-agent':'nbl-market-v1/1.2'},signal:AbortSignal.timeout(15_000),cf:{cacheTtl:label==='Odds events'?240:25,cacheEverything:true}});const {body}=await readJsonResponse(res,label);return {res,body,quota:oddsQuota(res)};}
...
async function fetchFeaturedOddsEvents(env){
  need(env?.ODDS_API_KEY,'ODDS_API_KEY is not configured');const q=new URLSearchParams({apiKey:env.ODDS_API_KEY,regions:'au',markets:'h2h',oddsFormat:'decimal',dateFormat:'iso'});const out=await oddsFetch(`${ODDS_HOST}/v4/sports/${ODDS_SPORT}/odds?${q}`,'Odds featured fallback');need(out.res.ok,`Odds featured fallback unavailable (${out.res.status})`);need(Array.isArray(out.body),'Odds featured fallback returned invalid payload');return out;
}
...
function eventDiagnostics(events,fixture){
  const tip=Date.parse(String(fixture?.start_time||''));return (Array.isArray(events)?events:[]).map(e=>({event_id:String(e?.id||''),home_team:String(e?.home_team||''),away_team:String(e?.away_team||''),commence_time:String(e?.commence_time||''),kickoff_delta_minutes:Number.isFinite(tip)&&Number.isFinite(Date.parse(String(e?.commence_time||'')))?Math.round((Date.parse(String(e.commence_time))-tip)/60000):null})).filter(x=>x.kickoff_delta_minutes!==null&&Math.abs(x.kickoff_delta_minutes)<=24*60).slice(0,12);
}
...
export async function fetchAndEvaluateOddsApi(input,env){
  ...
  const requested=requestedOddsMarkets(input),events=await fetchOddsEvents(env),fixture=state.run.lock?.fixture||state.freeze.fixture;let event,eventResolutionSource='events_endpoint',featured=null;const quota=[events.quota];
  try{event=resolveOddsEvent(events.body,fixture);}catch(primaryError){
    featured=await fetchFeaturedOddsEvents(env);quota.push(featured.quota);
  ...
  // Efficient first attempt: all requested markets in one event-scoped call. If The Odds API
  // rejects the bundle because one key is unsupported, probe each requested key once so NBL
  // partial support is detected instead of incorrectly classifying the entire sport as unsupported.
  ...
  const eventMeta={event_id:event.id,home_team:event.home_team,away_team:event.away_team,commence_time:event.commence_time};

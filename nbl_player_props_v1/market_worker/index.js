...
export async function fetchAndEvaluateOddsApi(input,env){
  ...
  const first=await fetchEventOdds(env,event,requested),quota=[events.quota,first.quota];let rows=[],marketsUsed=[],marketsUnsupported=[],fallbackProbed=false;
  ...
  if(marketsUsed.length===0)return {market_data:true,p_model_mutated:false,p_model_status:'FROZEN',run_id:state.runId,fixture_id:state.freeze.fixture_id,freeze_receipt_sha256:state.expected,frozen_at:state.freeze.frozen_at,odds_api_support:'UNSUPPORTED_MARKET',message:'The Odds API rejected every requested NBL assists/rebounds market key',event:{event_id:event.id,home_team:event.home_team,away_team:event.away_team,commence_time:event.commence_time},markets_requested:requested,markets_used:[],markets_unsupported:marketsUnsupported,fallback_probed_individually:fallbackProbed,quota,market_records_received:0,evaluation:null};
  if(rows.length===0)return {market_data:true,p_model_mutated:false,p_model_status:'FROZEN',run_id:state.runId,fixture_id:state.freeze.fixture_id,freeze_receipt_sha256:state.expected,frozen_at:state.freeze.frozen_at,odds_api_support:'SUPPORTED_EMPTY',event:{event_id:event.id,home_team:event.home_team,away_team:event.away_team,commence_time:event.commence_time},markets_requested:requested,markets_used:marketsUsed,markets_unsupported:marketsUnsupported,fallback_probed_individually:fallbackProbed,quota,market_records_received:0,evaluation:null};
  ...
  return {market_data:true,p_model_mutated:false,p_model_status:'FROZEN',run_id:state.runId,fixture_id:state.freeze.fixture_id,freeze_receipt_sha256:state.expected,frozen_at:state.freeze.frozen_at,odds_api_support:'SUPPORTED_WITH_ROWS',event:{event_id:event.id,home_team:event.home_team,away_team:event.away_team,commence_time:event.commence_time},markets_requested:requested,markets_used:marketsUsed,markets_unsupported:marketsUnsupported,fallback_probed_individually:fallbackProbed,quota,market_records_received:rows.length,evaluation};
}
